...
class PDFParser extends Parser with TextChunkerMixin, LLMPrompterMixin {
  ...
  Future<Result<List<Event>>> parseFile(
    ...
    if (filePath == null)
      return Result.error(Exception('PDF file path is null'));
    ...
    if (configuration?['allowCondense'] ?? false) {
      ...
      final List<String> chunks = chunkText(
        extractor.extractTextLines().map((line) => line.text).join('\n'),
        (progress) {
          onProgress?.call(
            ParseStage.STRIPPING,
            progress,
            'Extracting... ${(progress * 100).toInt()}%',
          );
        },
      );
    ...
    final Result<String> llmResult = await promptLLM(
      inputText,
    ...
    onProgress?.call(
      ...
      1.0,
      'Processing with AI... 100%',
    );

    final Result<ICalendar> iCalendarResult = gaurdedExectute(
      () => ICalendar.fromString((llmResult as Ok).value),
    );
    if (iCalendarResult is Error)
      return Result.error((iCalendarResult as Error).error);

    onProgress?.call(ParseStage.ICS_PARSING, 0.0, 'ICS Parsing...');
    return ICSParser.parseICalendar(
      ICalendar.fromString((llmResult as Ok).value),
      (double progress) {
        onProgress?.call(
