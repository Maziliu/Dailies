mixin TextChunkerMixin {
  ...
  List<String> _extractChunks(String content, {bool splitByLines = false}) {
    ...
    for (int i = 0; i < sentences.length; i++) {
      if (_isRelevantSentence(sentences[i])) {
        ...
        final after =
            (i < sentences.length - 1 && !splitByLines) ? sentences[i + 1] : '';
        final chunk = [
          before,
          sentences[i],
          after,
  ...
  bool _isRelevantChunk(String chunk) {
    ...
    if (RegExp(r'^\d{4}-\d{2}-\d{2}\s+\d+\s+of\s*\d*$').hasMatch(chunk.trim()))
      return false; //ex: "2024-08-29 1 of 9"

    //Remove chunks that are just "Electronically Approved" stamps
    if (RegExp(
      r'electronically\s+approved',
      caseSensitive: false,
    ).hasMatch(chunk))
      return false;
    ...
    final eventKeywords = RegExp(
        ...
        timeKeywords = RegExp(
          r'(at\s+\d{1,2}:\d{2}|pm|am|\d+\s+minutes?|hours?)',
          caseSensitive: false,
        );

    if (eventKeywords.hasMatch(chunk) || timeKeywords.hasMatch(chunk))
      return true;

    if (RegExp(
      r'(week|assignment|lab)\s+\d+',
      caseSensitive: false,
    ).hasMatch(chunk))
      return true; //structuered data like tables and shcedules
