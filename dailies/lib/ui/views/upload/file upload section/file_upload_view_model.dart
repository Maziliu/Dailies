...
class FileUploadViewModel extends ChangeNotifier with ErrorStreamMixin {
  ...
  final StreamController<Map<String, ParseProgress>> _progressController =
      StreamController<Map<String, ParseProgress>>.broadcast();

  Stream<Map<String, ParseProgress>> get progressStream =>
      _progressController.stream;

  Map<String, ParseProgress> _parseProgress = {};
  Map<String, ParseProgress> get parseProgress =>
      Map.unmodifiable(_parseProgress);

  late final Future<void> Function(
    List<PlatformFile>,
    Function(String, ParseProgress),
  )
  parseFilesCallback;
  ...
  void removeUploadedFile(PlatformFile fileToRemove) {
    uploadedFiles.value =
        uploadedFiles.value.where((file) => file != fileToRemove).toList();
