...
class ICSParser extends Parser {
  ...
  Future<Result<List<Event>>> parseFile(
    ...
    String? filePath, {
    Function(ParseStage, double, String?)? onProgress,
  }) async {
    if (filePath == null)
      return Result.error(Exception('ICS file path is null'));

    final File icsFile = File(filePath);
    if (!icsFile.existsSync())
      return Result.error(Exception('ICS file does not exist $filePath'));
    ...
    try {
      ...
      return Result.error(
        Exception('Failed to read file content: ${exception.toString()}'),
      );
    }
    ...
    try {
      ...
      return Result.error(
        Exception('Failed to parse ICS file: ${e.toString()}'),
      );
    }
    ...
    return parseICalendar(iCalendar, (double progress) {
      onProgress?.call(
        ...
        progress,
        'ICS Parsing... ${(progress * 100).toInt()}%',
      );
    });
  }

  static Result<List<Event>> parseICalendar(
    ICalendar iCalendar,
    ...
    for (final eventData in iCalendar.data) {
      ...
      final Result result = gaurdedExectute(() {
        ...
        final List? exclusionDates =
            eventData['exdate']?.map((date) => date.toDateTime()).toList() ??
            [];

        final Event event = Event(
          eventName: summary ?? 'Untitled',
          location: location,
        );
        ...
        if (recurranceRule != null) {
          ...
          final TimeSlot reference = TimeSlot.UnSaved(
            dateOfTimeSlot: startDate,
            startTime: startTime,
            endTime: endTime,
          );
          ...
          pattern = TimeSlotPattern.UnSaved(
            endPatternDate:
                (endTime != null) ? normalizeDateTime(endTime) : null,
            exclusionDates:
                exclusionDates?.map((date) => date as DateTime).toList(),
